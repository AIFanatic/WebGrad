...
import { Tensor, Random, nn, Device } from "../../src/";
import { TestRunner } from "../run-web";

function NNTest(device: Device) {

    TestRunner.describe("Linear", () => {
        let linear = new nn.Linear(2, 4);
        linear.weight = new Tensor([[-0.5963, -0.0062],[ 0.1741, -0.1097],[-0.4237, -0.6666],[ 0.1204, 0.2781]], { requires_grad: true});
        linear.bias = new Tensor([-0.4580, -0.3401, 0.2950, 0.1145], { requires_grad: true});
        linear.to(device);
        
        const input = new Tensor([[1,2], [3,4]], {device: device, requires_grad: true});
    ...
    TestRunner.describe("Dropout", () => {
        ...
        const dropout = new nn.Dropout().to(device);
        x = dropout.forward(x);
    ...
    TestRunner.describe("LayerNorm", () => {
        const x = Tensor.arange(0, 10, 1, {device: device});
        const layerNorm = new nn.LayerNorm([10]).to(device);
    ...
    TestRunner.describe("Softmax", () => {
        ...
        const softmax = new nn.Softmax(0).to(device);
        ...
        const x1 = new Tensor([[5.0, 6.0, 3.0]], {device: device});
        const softmax1 = new nn.Softmax(1).to(device);
        ...
        const softmax2 = new nn.Softmax(-1).to(device);
    ...
    TestRunner.describe("Embedding", () => {
        ...
        let embedding = new nn.Embedding(10, 3);
        ...
        const input = new Tensor([[1, 2, 4, 5], [4, 3, 2, 9]], {device: device});
...
export const NNTests = {category: "Layers", func: NNTest};