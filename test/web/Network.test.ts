...
import { Device, nn } from "../../src/";
import { Random } from "../../src/Random";
import { TestRunner } from "../run-web";

function NetworkTest(device: Device) {
    TestRunner.describe("1 dense layer", () => {
        ...
        const yb = new Tensor([[1.0], [0.2], [0.3], [0.7]], {device: device, requires_grad: true});
        ...
        for (let p of model.parameters()) {
            p.data = p.sub(new Tensor(p.grad, {device: device}).mul(learning_rate)).data;
        }
    ...
    TestRunner.describe("3 dense layers", () => {
        ...
        function get_loss(model: SimpleModel, x_tensor: Tensor, y_tensor: Tensor): Tensor {
            ...
            // Reg loss
            let reg_loss = new Tensor([0], {device: device, requires_grad: true});
            for (let p of model.parameters()) {
                const w_sum = p.mul(p).sum();
                const p_shape_prod = p.data.shape.reduce((p, c) => p * c);
                const div = w_sum.div(new Tensor(p_shape_prod, {device: device, requires_grad: true}));
                reg_loss = reg_loss.add(div);
            }
    
            
            const alpha = new Tensor(1e-4, {device: device, requires_grad: true});
        ...
        const Y = new Tensor([1.0, 0.2, 0.3, 0.7], {device: device, requires_grad: true});
...
export const NetworkTests = {category: "Network", func: NetworkTest};