...
import { BinaryOp } from "../../src/backend/BinaryOps";

function TensorGradTest(device: Device) {

    TestRunner.describe("Add", () => {
        const a = new Tensor([1,2,3], {device: device, requires_grad: true});
        const b = new Tensor([4,5,6], {device: device, requires_grad: true});
        const c = a.add(b);
    ...
    TestRunner.describe("Pow", () => {
        const a = new Tensor([4,5,6], {device: device, requires_grad: true});
        const b = a.pow(2);
    ...
    TestRunner.describe("Permute", () => {
        const a = new Tensor([[1, 2], [3, 4], [5, 6]], {device: device, requires_grad: true});
    ...
    TestRunner.describe("Abs", () => {
        const a = new Tensor([[-3, 3, 3], [4, -4, 4]], {device: device, requires_grad: true});
        const b = a.abs();
    ...
    TestRunner.describe("Simple model", () => {
        const weight = new Tensor([[-0.4869, -0.0896], [-0.0051, -0.3460], [ 0.1421, -0.5443]], {device: device, requires_grad: true});
    
        let x = new Tensor([[2.0, 3.0, -1.0]], {device: device, requires_grad: true});
        x = x.matmul(weight);
    ...
    TestRunner.describe("Maximum", () => {
        const a = new Tensor([2, 3, 4], {device: device, requires_grad: true});
        const b = new Tensor([1, 5, 2], {device: device, requires_grad: true});
        const c = a.maximum(b);
    ...
    TestRunner.describe("Equal", () => {
        const a = new Tensor([1, 2, 3], {device: device, requires_grad: true});
        const b = new Tensor([0, 2, 2], {device: device, requires_grad: true});
        const c = a.eq(b);
...
export const TensorGradTests = {category: "Tensor Grad", func: TensorGradTest};